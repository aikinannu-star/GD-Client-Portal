	...
	function gd_client_portal_marketplace_get_customer_orders($user_id = 0, $limit = 20)
	{
		...
		if ($user_id || !empty($user_email)) {
			...
			foreach ($results as $order) {
				if ($user_id && absint($order->post_author) !== $user_id) {
					continue;
				}
				$meta_tenant_id = absint(get_post_meta($order->ID, '_gd_mp_order_tenant_id', true));
				if ($current_tenant_id > 0 && $meta_tenant_id !== 0 && $meta_tenant_id !== $current_tenant_id) {
					continue;
				}
				$meta_email = get_post_meta($order->ID, '_gd_mp_order_email', true);
				if (!empty($user_email) && !empty($meta_email) && sanitize_email($meta_email) !== $user_email) {
					continue;
				}
		...
		if (is_array($order_log)) {
			foreach ($order_log as $reference => $entry) {
				...
				if ($current_tenant_id > 0) {
					...
					if ($entry_tenant_id !== 0 && $entry_tenant_id !== $current_tenant_id) {
						continue;
					}
				}
				$entry_email = sanitize_email($entry['email'] ?? '');
				if ($user_id && !empty($entry['user_id']) && absint($entry['user_id']) !== $user_id) {
					continue;
				}
				if (!empty($user_email) && !empty($entry_email) && $entry_email !== $user_email) {
					continue;
				}
...
if (!function_exists('gd_client_portal_render_marketplace')) {
	...
	if (!function_exists('gd_client_portal_marketplace_add_to_cart')) {
		...
		add_action('wp_ajax_nopriv_gd_client_portal_marketplace_add_to_cart', 'gd_client_portal_marketplace_add_to_cart');
	}
}
...
if (!function_exists('gd_client_portal_marketplace_create_paystack_checkout')) {
	...
	add_action('wp_ajax_nopriv_gd_client_portal_marketplace_create_paystack_checkout', 'gd_client_portal_marketplace_create_paystack_checkout');
}
