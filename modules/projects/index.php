...
if (!function_exists('gd_client_portal_register_projects_module')) {
	function gd_client_portal_register_projects_module()
	{
		...
		add_shortcode('gd_service_projects', 'gd_client_portal_render_projects');
...
if (!function_exists('gd_client_portal_render_projects')) {
	function gd_client_portal_render_projects($atts = array())
	{
		...
		// Enqueue assets for the module
		wp_enqueue_style('gd-client-portal-projects');
		wp_enqueue_script('gd-client-portal-projects');

		$user_id = get_current_user_id();
		// For demo purposes: pull posts of type 'project' if registered, fallback to a simple placeholder.
		$projects = array();
		if (post_type_exists('project')) {
			$q = new WP_Query(array('post_type' => 'project', 'author' => $user_id, 'post_status' => 'publish', 'posts_per_page' => -1));
			if ($q->have_posts()) {
				while ($q->have_posts()) {
					$q->the_post();
					$projects[] = array('ID' => get_the_ID(), 'title' => get_the_title(), 'link' => get_permalink());
				}
				wp_reset_postdata();
			}
		}

		// Load module template if exists
		$tpl = GD_CLIENT_PORTAL_PATH . 'modules/projects/templates/list.php';
		if (file_exists($tpl)) {
			ob_start();
			include $tpl;
			return ob_get_clean();
		...
		if (empty($projects)) {
			return '<p>' . esc_html__('No projects found.', 'gd-client-portal') . '</p>';
		}

		$out = '<ul class="gd-project-list">';
		foreach ($projects as $p) {
			$out .= '<li><a href="' . esc_url($p['link']) . '">' . esc_html($p['title']) . '</a></li>';
		}
		$out .= '</ul>';
